...
dnl
dnl based on http://svn.apache.org/repos/asf/incubator/thrift/trunk/aclocal/aclocal/ax_lib_event.m4
dnl
...
AC_DEFUN([AX_LIB_EVENT],
         [
          ...
          AC_ARG_WITH([libeventdir],
                      AS_HELP_STRING([--with-libeventdir@<:@=DIR@:>@], [specify an alternate root directory for libevent]),
                      [
          ...
          if test "$want_libevent" = "yes"; then
            ...
            if test -n "$ax_libevent_path"; then
              ...
              for ax_libevent_path in "" "$LIBEVENT_ROOT" ; do
                AX_LIB_EVENT_DO_CHECK
